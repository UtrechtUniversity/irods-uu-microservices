...
int calculate_checksum(rsComm_t *rsComm, std::string const& dataObjInp, std::string replNum, char* outChksum) {
    ...
    if ( status >= 0 ) {
        if (genQueryOut->rowCnt != 1) {
            ...
            if (( path = getSqlResultByInx( genQueryOut, COL_D_DATA_PATH ) ) == NULL) {
                rodsLog( LOG_ERROR, "_msi_dataObj_checksum: getSqlResultByInx for COL_D_DATA_PATH failed");
    ...
    if (retCode < 0) {
        rodsLog( LOG_ERROR, "_msi_dataObj_checksum: Failed to calculate checksum for file: [{}], status = {}", myData, retCode);
        ret = retCode;
    }
...
int msidataObjchecksum(
    msParam_t* dataObjInp,
    msParam_t* replNum,
    msParam_t* statusOut,
    ruleExecInfo_t* rei ) {
...
irods::ms_table_entry* plugin_factory() {
    ...
    msvc->add_operation<
        ...
        msParam_t*,
        msParam_t*,
        ruleExecInfo_t*>("msidataObjchecksum",
                         std::function<int(
                             msParam_t*,
                             msParam_t*,
                             msParam_t*,
