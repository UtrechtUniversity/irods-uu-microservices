...
int msiRegisterEpicPID(msParam_t* valueIn, msParam_t* idInOut, msParam_t* httpCodeOut, ruleExecInfo_t* rei)
{
    CURL* curl;
    CURLcode res;
    ...
    if (curl) {
        ...
        /* Create payload. */
        std::string payload =
            "{\"values\":[{\"index\":1,\"type\":\"URL\",\"data\":{\"format\":\"string\",\"value\":\"" +
            escapeJson(value) +
            "\"}},{\"index\":100,\"type\":\"HS_ADMIN\",\"data\":{\"format\":\"admin\",\"value\":{\"handle\":\"0.NA/" +
            prefix + "\",\"index\":200,\"permissions\":\"011111110011\"}}}]}";
        ...
        if (res != CURLE_OK) {
            ...
            /* 400 Bad Request */
                ...
                rodsLog(LOG_ERROR, "msiRegisterEpicPID: Invalid handle");
            }
            /* 401 Unauthorized */
            else if (http_code == 401) {
                rodsLog(LOG_ERROR, "msiRegisterEpicPID: Authentication needed");
            }
            /* 403 Forbidden */
            else if (http_code == 403) {
                rodsLog(LOG_ERROR, "msiRegisterEpicPID: Permission denied");
            }
            /* 404 Not Found */
            else if (http_code == 404) {
                rodsLog(LOG_ERROR, "msiRegisterEpicPID: Handle not found");
            }
            /* 409 Conflict */
            else if (http_code == 409) {
                rodsLog(LOG_ERROR, "msiRegisterEpicPID: Handle or value already exists");
            }
            /* 500 Internal Server Error */
            else if (http_code == 500) {
                rodsLog(LOG_ERROR, "msiRegisterEpicPID: Server internal error");
            }
            else {
                rodsLog(LOG_ERROR, "msiRegisterEpicPID: HTTP error code: %lu", http_code);
            }
        }
...
irods::ms_table_entry* plugin_factory()
{
    ...
    msvc->add_operation<msParam_t*, msParam_t*, msParam_t*, ruleExecInfo_t*>(
        "msiRegisterEpicPID",
        std::function<int(msParam_t*, msParam_t*, msParam_t*, ruleExecInfo_t*)>(msiRegisterEpicPID));
