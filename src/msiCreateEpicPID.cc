...
extern "C" {
		       ...
		       ruleExecInfo_t *rei)
  {
    ...
    if(curl) {
      ...
      /* Check for errors. */
      if(res != CURLE_OK) {
	rodsLog(LOG_ERROR, "msiCreateEpicPID: curl error: %s", curl_easy_strerror(res));
	...
	/* 400 Bad Request */
	  ...
	  rodsLog(LOG_ERROR,
		  "msiCreateEpicPID: Invalid handle");
	}
	/* 401 Unauthorized */
	else if (http_code == 401) {
	  rodsLog( LOG_ERROR,
		   "msiCreateEpicPID: Authentication needed");
	}
	/* 403 Forbidden */
	else if (http_code == 403) {
	  rodsLog(LOG_ERROR,
		   "msiCreateEpicPID: Permission denied");
	}
	/* 404 Not Found */
	else if (http_code == 404) {
	  rodsLog(LOG_ERROR,
		   "msiCreateEpicPID: Handle not found");
	}
	/* 409 Conflict */
	else if (http_code == 409) {
	  rodsLog(LOG_ERROR,
		   "msiCreateEpicPID: Handle or value already exists");
	}
        /* 500 Internal Server Error */
        else if (http_code == 500) {
	  rodsLog(LOG_ERROR,
		   "msiCreateEpicPID: Server internal error");
        }
	else {
	  rodsLog(LOG_ERROR,
		  "msiCreateEpicPID: HTTP error code: %lu", http_code);
  	}
      }
